...
To install this tool, run `dart pub global activate df_generate_dart_models 0.12.1` in your terminal. You can replace `0.12.1` with any other version or omit it to install the latest version. It’s recommended to specify a version to avoid potential issues from frequent updates and changes.
...
```yaml
dependencies:
  df_generate_dart_models_core: 0.8.0 # Fixed version to ensure compatibility and avoid issues from frequent updates.
```
